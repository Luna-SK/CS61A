...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        if value.rest:
            env.bindings[signature] = scheme_eval(value, env)
        else:
            env.bindings[signature] = scheme_eval(value.first, env)
        return signature
        # END PROBLEM 4
...
def do_and_form(expressions, env):
    ...
    if not expressions:
        ...
        if is_scheme_false(first_result):
            return first_result
        else:
...
def do_or_form(expressions, env):
    ...
    if not expressions:
        ...
        first_result = scheme_eval(expressions.first, env)
        if is_scheme_true(first_result):
            return first_result
        else:
