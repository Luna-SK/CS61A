...
class Coin:
    ...
    def worth(self):
        extra_year = Mint.present_year - self.year
        if extra_year:
...
def two_list(vals, counts):
    ...
    """
    lst = Link.empty
    for time, value in zip(counts[::-1], vals[::-1]):
        for _ in range(time):
            lst = Link(value, lst)
    return lst
...
class VirFib():
    ...
    def next(self):
        if self.value == 0:
            ...
            result = VirFib(self.value + self.pre_value)
            result.pre_value = self.value
            return result
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        return all([is_bst(b) for b in t.branches]) and max_bst(t.branches[0]) <= t.label and min_bst(t.branches[1]) > t.label
    else:
        return False
