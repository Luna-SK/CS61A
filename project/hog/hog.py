...
def tail_points(opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    return 2 * abs(opponent_score % 10 - opponent_score % 100 // 10) + 1
    # END PROBLEM 2
...
def perfect_square(score):
    """Judge if the resulting score is a perfect square"""
    k = 1
    while k * k < score:
        k += 1
    return k * k == score
...
def next_perfect_square(score):
    """increase score to the next higher perfect square"""
    k = 1
    while k * k <= score:
        k += 1
    return k * k
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while score < goal:
        ...
        while opponent_score < goal:
            if num_dice != strategy(score, opponent_score):
                return False
            opponent_score += 1
        score += 1
    return True
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    max_score, min_num_rolls, num = 0, 0, 1
    while num <= 10:
        score = make_averaged(roll_dice, total_samples)(num, dice)
        if score > max_score:
            max_score = score
            min_num_rolls = num
        num += 1
...
def tail_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    score0 = tail_points(opponent_score)
    if score0 >= threshold:
        return 0
    else:
        return num_rolls
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    score0 = square_update(0, score, opponent_score) - score
    if score0 >= threshold:
        return 0
    else:
        return num_rolls
